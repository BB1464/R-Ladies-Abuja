...
---
- `Aestetic` properties in R but this can be specified within the scale_* function.
```{r,fig.align='center',fig.width=12,fig.height=5,echo=FALSE}
knitr::include_graphics(path = 'img/common-aesthetics-1.png')
```
---
- Shapes that are available in R which can also be specified within the scale_* function.
...
```
---
- Old approach to apply aesthetics mapping after the statistics
```{r,fig.align='center',fig.width=12,fig.height=6}
ggplot(data = mpg,mapping = aes(x=class,y=stat(100*..count../sum(..count..))))+geom_bar()
```
---
- `after_scale` function which is the new approach
```{r,fig.align='center',fig.width=12,fig.height=6}
...
- I will like to thank **@R-Ladies Abuja** organisers for giving me the opportunity to present this presentation.
